...
from config import num_gaussians, lstm_hidden_layer_size, dropout, mlp_hidden_dim, l2_lambda
...
class MDNLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        pi = F.softmax(self.pi(x) / 1, dim=1)  # Example temperature T=0.1 to sharpen
        sigma = F.softplus(self.sigma(x))  # Ensures sigma is positive
        mu = F.softplus(self.mu(x))  # Ensures mu is positive
